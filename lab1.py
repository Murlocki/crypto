...
def euler_function_type1(x):
    count = 0
    for i in range(1,x):
        if math.gcd(i,x) == 1:
            count += 1
    return count
def euler_function_type2(x):
    def is_prime(n):
        result = True
    ...
    for i in range(2,x+1):
        if is_prime(i) and x%i == 0:
            simple_dels.append(i)
    return int(x * reduce(lambda x, y: x * y, (1-1/i for i in simple_dels),1))
#print(euler_function_type1(15))
#print(euler_function_type2(15))
def solve_linear_congruence(a, b, m):
    ...
    return [f'{sol}+{m}*k'for sol in sorted(solutions)]
#print(solve_linear_congruence(8, 0, 12))
...
mod_arith = ModularArithmetic(15)  # Работаем по модулю 17 (простое число)
#print(mod_arith.add(10, 9))  # 19 % 17 = 2
